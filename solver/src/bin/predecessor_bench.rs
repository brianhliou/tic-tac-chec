...
fn main() {
    ...
    let mut edges = 0_u64;
    let mut zero_predecessor_states = 0_u64;
    let mut checksum = 0_u64;
    let start = Instant::now();
    for index in 0..iterations {
        let id = black_box(ids[index as usize % ids.len()]);
        let count = for_each_predecessor(id, Rules::default(), |parent| {
            checksum = checksum.wrapping_add(black_box(parent.get()) as u64);
        });
        edges += count as u64;
        zero_predecessor_states += u64::from(count == 0);
    }
    let elapsed = start.elapsed();

    println!("iterations = {iterations}");
    println!("edges = {edges}");
    println!("zero_predecessor_states = {zero_predecessor_states}");
    println!(
        "mean_predecessors_per_state = {:.3}",
